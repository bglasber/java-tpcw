...
public class CommitExecutor implements Callable<commit_result> {
  private Logger log = LoggerFactory.getLogger(this.getClass());
  private DMSiteManagerClient client;
  private int clientId;

  public CommitExecutor(DMSiteManagerClient client, int clientId) {
    this.client = client;
  ...
  public commit_result commit() {
    ...
    try {
      ...
      res = client.commitTransaction(clientId);
    } catch (TException x) {
      log.error("Error committing transaction:{}", x.getMessage());
