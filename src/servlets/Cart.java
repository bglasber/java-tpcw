...
public class Cart {
    ...
    public Cart (DMResultSet rs, double C_DISCOUNT) throws java.sql.SQLException{
        int i;
        int total_items;
        lines = new Vector();
        while (rs.next()) { // While there are lines remaining
          CartLine line =
              new CartLine(rs.getString("i_title"), rs.getDouble("i_cost"),
                           rs.getDouble("i_srp"), rs.getString("i_backing"),
                           rs.getInt("scl_qty"), rs.getInt("scl_i_id"));
          lines.addElement(line);
        }
		rs.close();
    ...
    public Cart (JSONArray rs, double C_DISCOUNT) throws JSONException{
        int i;
        int total_items;
        lines = new Vector();
        for(int j = 0; j < rs.length(); j++){//While there are lines remaining
            JSONObject obj = rs.getJSONObject(j);
            CartLine line = new CartLine(obj.getString("i_title"),
                    obj.getDouble("i_cost"),
                    obj.getDouble("i_srp"),
                    obj.getString("i_backing"),
                    obj.getInt("scl_qty"),
                    obj.getInt("scl_i_id"));
            lines.addElement(line);
        }
