...
public class TPCW_shopping_cart_interaction extends HttpServlet {
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse res)
    ...
    if (SHOPPING_IDstr == null) {
      BeginShoppingCartResult b = TPCW_DM.beginShoppingCartWithEmptyCart(eb_id);
      writeLocations = b.writeLocations;
      SHOPPING_ID = TPCW_DM.createEmptyCartWithinTxn(eb_id, writeLocations, b.shoppingId);
    } else {
      SHOPPING_ID = Integer.parseInt(SHOPPING_IDstr);
      writeLocations = TPCW_DM.beginShoppingCart(eb_id, SHOPPING_ID);
    }

    cart = TPCW_DM.doCartWithinTxn(eb_id, writeLocations, SHOPPING_ID, I_ID, ids, quantities);
    ...
    TPCW_promotional_processing.DisplayPromotions(out, req, res, SHOPPING_ID);

    TPCW_DM.commit(eb_id);

    // Display the shopping cart contents
    out.print("<FORM ACTION=\"TPCW_shopping_cart_interaction" +
              TPCW_Util.sessionIdString + req.getRequestedSessionId() +
              "\" METHOD=\"get\">\n");
